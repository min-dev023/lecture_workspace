...
public class Gallery2 {
	...
	JPanel p_container; // 북쪽, 큰 사진 패널들을 감싸안들 바깥 쪽 컨테이너(BorderLayout)
	...
	// 화면에 렌더링하지는 않지만, 원하는 좌표에 사각형을 메모리상에 존재시키면, 원하는 영역에 이벤트를 부여할 수 있다.
	Rectangle[] rectArray = new Rectangle[imgArray.length];
	
	float y = 0;
	
	Thread thread; // ex) js의 게임 루프
	
	float a = 0.1f;
	int targetY;
	int currentIndex; // 현재 유저가 클릭한 이미지의 index
	...
	public void createImage() {
		for(int i = 0; i < 9; i++) {
			...
			rectArray[i] = new Rectangle(5, 10 + (90*i), 90, 90);
		}
	}
	
	// 포인터가 목적지에 한번에 도달하게 하지말고, 감속도 공식을 적용하여 움직이게 함
	public void move() {
		// y값은 현y값 + a * (목적 y지점 - 현y)
		y = y + a * (targetY - y);
